...
And indeed, the resulting app features a `ComboBoxText` with a red arrow.

<div style="text-align:center"><img src="img/css_7.png"/></div>

## Images

But do we really want to settle with an arrow?
...
Before the individual tasks were a bit hard to distinguish so let us change that!
...
Our To-Do app should represent a real world app, so we will not be satisfied by choosing yellow and calling it a day.
...
Perfect!
We can access exported colors by adding a `@` in front of their names.
