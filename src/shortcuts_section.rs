    ...
    pub fn connect_change_current_page<F: Fn(&ShortcutsSection, i32) -> bool + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"change-current-page\0".as_ptr() as *const _,
                Some(transmute(change_current_page_trampoline::<F> as usize)), Box::into_raw(f))
        }
    }

    pub fn emit_change_current_page(&self, object: i32) -> bool {
        let stash: Stash<*mut gtk_sys::GtkShortcutsSection, _> = self.to_glib_none();
        let gobject = unsafe { glib::Object::from_glib_borrow(stash.0 as *mut gobject_sys::GObject) };
