...
impl WidgetPaintable {
    pub fn new<P: IsA<Widget>>(widget: Option<&P>) -> WidgetPaintable {
        ...
        unsafe {
            from_glib_full(gtk_sys::gtk_widget_paintable_new(widget.map(|p| p.as_ref()).to_glib_none().0))
        }
    }
}
...
impl<O: IsA<WidgetPaintable>> WidgetPaintableExt for O {
    fn get_widget(&self) -> Option<Widget> {
        unsafe {
            from_glib_none(gtk_sys::gtk_widget_paintable_get_widget(self.as_ref().to_glib_none().0))
        }
    }

    fn set_widget<P: IsA<Widget>>(&self, widget: Option<&P>) {
        unsafe {
            gtk_sys::gtk_widget_paintable_set_widget(self.as_ref().to_glib_none().0, widget.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
...
unsafe extern "C" fn notify_widget_trampoline<P, F: Fn(&P) + 'static>(this: *mut gtk_sys::GtkWidgetPaintable, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
