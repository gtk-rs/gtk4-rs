...
impl MediaFile {
    ...
    pub fn new_for_input_stream<P: IsA<gio::InputStream>>(stream: Option<&P>) -> MediaFile {
        assert_initialized_main_thread!();
        unsafe {
            MediaStream::from_glib_full(gtk_sys::gtk_media_file_new_for_input_stream(stream.map(|p| p.as_ref()).to_glib_none().0)).unsafe_cast()
        }
    }
...
impl<O: IsA<MediaFile>> MediaFileExt for O {
    ...
    fn get_input_stream(&self) -> Option<gio::InputStream> {
        unsafe {
            from_glib_none(gtk_sys::gtk_media_file_get_input_stream(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn set_input_stream<P: IsA<gio::InputStream>>(&self, stream: Option<&P>) {
        unsafe {
            gtk_sys::gtk_media_file_set_input_stream(self.as_ref().to_glib_none().0, stream.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
