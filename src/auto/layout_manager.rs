...
pub trait LayoutManagerExt: 'static {
    ...
    fn measure<P: IsA<Widget>>(&self, widget: &P, orientation: Orientation, for_size: i32) -> (i32, i32, i32, i32);
}

impl<O: IsA<LayoutManager>> LayoutManagerExt for O {
    fn allocate<P: IsA<Widget>>(&self, widget: &P, width: i32, height: i32, baseline: i32) {
        unsafe {
            gtk_sys::gtk_layout_manager_allocate(self.as_ref().to_glib_none().0, widget.as_ref().to_glib_none().0, width, height, baseline);
        }
    }

    fn get_layout_child<P: IsA<Widget>>(&self, child: &P) -> Option<LayoutChild> {
        unsafe {
            from_glib_none(gtk_sys::gtk_layout_manager_get_layout_child(self.as_ref().to_glib_none().0, child.as_ref().to_glib_none().0))
        }
    }

    fn get_request_mode(&self) -> SizeRequestMode {
        unsafe {
            from_glib(gtk_sys::gtk_layout_manager_get_request_mode(self.as_ref().to_glib_none().0))
        }
    }

    fn get_widget(&self) -> Option<Widget> {
        unsafe {
            from_glib_none(gtk_sys::gtk_layout_manager_get_widget(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn measure<P: IsA<Widget>>(&self, widget: &P, orientation: Orientation, for_size: i32) -> (i32, i32, i32, i32) {
        unsafe {
            ...
            gtk_sys::gtk_layout_manager_measure(self.as_ref().to_glib_none().0, widget.as_ref().to_glib_none().0, orientation.to_glib(), for_size, &mut minimum, &mut natural, &mut minimum_baseline, &mut natural_baseline);
