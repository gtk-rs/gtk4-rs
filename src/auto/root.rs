...
impl<O: IsA<Root>> RootExt for O {
    fn get_focus(&self) -> Option<Widget> {
        unsafe {
            from_glib_none(gtk_sys::gtk_root_get_focus(self.as_ref().to_glib_none().0))
        }
    }

    fn set_focus<P: IsA<Widget>>(&self, focus: Option<&P>) {
        unsafe {
            gtk_sys::gtk_root_set_focus(self.as_ref().to_glib_none().0, focus.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
    ...
    fn get_property_focus_widget(&self) -> Option<Widget> {
        unsafe {
            ...
            gobject_sys::g_object_get_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"focus-widget\0".as_ptr() as *const _, value.to_glib_none_mut().0);
            value.get()
        }
    }
    ...
    fn set_property_focus_widget(&self, focus_widget: Option<&Widget>) {
        unsafe {
            gobject_sys::g_object_set_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"focus-widget\0".as_ptr() as *const _, Value::from(focus_widget).to_glib_none().0);
        }
    }
...
unsafe extern "C" fn notify_focus_widget_trampoline<P, F: Fn(&P) + 'static>(this: *mut gtk_sys::GtkRoot, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
where P: IsA<Root> {
