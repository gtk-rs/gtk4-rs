...
impl Clipboard {
    ...
    pub fn read_text_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<GString, Error>) + Send + 'static>(&self, cancellable: Option<&P>, callback: Q) {
        ...
        unsafe extern "C" fn read_text_async_trampoline<Q: FnOnce(Result<GString, Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            ...
            let ret = gdk_sys::gdk_clipboard_read_text_finish(_source_object as *mut _, res, &mut error);
        ...
        unsafe {
            gdk_sys::gdk_clipboard_read_text_async(self.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
    ...
    pub fn read_texture_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<Texture, Error>) + Send + 'static>(&self, cancellable: Option<&P>, callback: Q) {
        let user_data: Box<Q> = Box::new(callback);
        unsafe extern "C" fn read_texture_async_trampoline<Q: FnOnce(Result<Texture, Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            let mut error = ptr::null_mut();
            let ret = gdk_sys::gdk_clipboard_read_texture_finish(_source_object as *mut _, res, &mut error);
            let result = if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) };
            let callback: Box<Q> = Box::from_raw(user_data as *mut _);
            callback(result);
        }
        ...
        unsafe {
            gdk_sys::gdk_clipboard_read_texture_async(self.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
    ...
    pub fn read_value_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<glib::Value, Error>) + Send + 'static>(&self, type_: glib::types::Type, io_priority: glib::Priority, cancellable: Option<&P>, callback: Q) {
        ...
        unsafe extern "C" fn read_value_async_trampoline<Q: FnOnce(Result<glib::Value, Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            let mut error = ptr::null_mut();
            let ret = gdk_sys::gdk_clipboard_read_value_finish(_source_object as *mut _, res, &mut error);
        ...
        unsafe {
            gdk_sys::gdk_clipboard_read_value_async(self.to_glib_none().0, type_.to_glib(), io_priority.to_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
    ...
    pub fn store_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<(), Error>) + Send + 'static>(&self, io_priority: glib::Priority, cancellable: Option<&P>, callback: Q) {
        let user_data: Box<Q> = Box::new(callback);
        unsafe extern "C" fn store_async_trampoline<Q: FnOnce(Result<(), Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            gdk_sys::gdk_clipboard_store_async(self.to_glib_none().0, io_priority.to_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }

    #[cfg(feature = "futures")]
    pub fn store_async_future(&self, io_priority: glib::Priority) -> Box_<dyn future::Future<Output = Result<(), Error>> + std::marker::Unpin> {
        use gio::GioFuture;
        use fragile::Fragile;
