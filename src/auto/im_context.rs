...
impl<O: IsA<IMContext>> IMContextExt for O {
    ...
    //fn filter_keypress(&self, event: /*Ignored*/&mut gdk::EventKey) -> bool {
    //    unsafe { TODO: call ffi::gtk_im_context_filter_keypress() }
    //}
    ...
    fn set_client_widget<P: IsA<Widget>>(&self, widget: Option<&P>) {
        unsafe {
            ffi::gtk_im_context_set_client_widget(self.as_ref().to_glib_none().0, widget.map(|p| p.as_ref()).to_glib_none().0);
        }
    }

    //fn set_cursor_location(&self, area: /*Ignored*/&gdk::Rectangle) {
    //    unsafe { TODO: call ffi::gtk_im_context_set_cursor_location() }
    //}
