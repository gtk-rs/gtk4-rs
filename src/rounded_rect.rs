...
impl RoundedRect {
    ...
    pub fn init_from_rect(&mut self, bounds: Rect, radius: f32) {
        unsafe {
            gsk_sys::gsk_rounded_rect_init_from_rect(&mut self.0, bounds.to_glib_none().0, radius);
        }
    }

    pub fn normalize(&mut self) {
        unsafe {
            gsk_sys::gsk_rounded_rect_normalize(&mut self.0);
        }
    }

    pub fn offset(&mut self, dx: f32, dy: f32) {
        unsafe {
            gsk_sys::gsk_rounded_rect_offset(&mut self.0, dx, dy);
        }
    }

    pub fn shrink(&mut self, top: f32, right: f32, bottom: f32, left: f32) {
        unsafe {
            gsk_sys::gsk_rounded_rect_shrink(&mut self.0, top, right, bottom, left);
        }
    }
...
impl<'a> ToGlibPtr<'a, *const gsk_sys::GskRoundedRect> for RoundedRect {
    ...
    fn to_glib_none(&'a self) -> Stash<*const gsk_sys::GskRoundedRect, Self> {
        Stash(&self.0, self)
    }
}
