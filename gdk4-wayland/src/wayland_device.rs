...
impl<O: IsA<WaylandDevice>> WaylandDeviceManualExt for O {
    fn get_wl_keyboard(&self) -> WlKeyboard {
        unsafe {
            let ptr =
                gdk_wayland_sys::gdk_wayland_device_get_wl_keyboard(self.as_ref().to_glib_none().0);
    ...
    fn get_wl_pointer(&self) -> WlPointer {
        unsafe {
            let ptr =
                gdk_wayland_sys::gdk_wayland_device_get_wl_pointer(self.as_ref().to_glib_none().0);
            Proxy::from_c_ptr(ptr as *mut wl_proxy).into()
        }
    }
    ...
    fn get_wl_seat(&self) -> WlSeat {
        unsafe {
            let ptr =
                gdk_wayland_sys::gdk_wayland_device_get_wl_seat(self.as_ref().to_glib_none().0);
            Proxy::from_c_ptr(ptr as *mut wl_proxy).into()
        }
    }
}
