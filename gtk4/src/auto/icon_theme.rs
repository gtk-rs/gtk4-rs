...
impl IconTheme {
    ...
    pub fn resource_path(&self) -> Option<Vec<glib::GString>> {
        unsafe {
            TryFromGlibPtrContainer::try_from_glib_full(ffi::gtk_icon_theme_get_resource_path(
                self.to_glib_none().0,
            ))
        }
    }
    ...
    pub fn search_path(&self) -> Option<Vec<std::path::PathBuf>> {
        unsafe {
            TryFromGlibPtrContainer::try_from_glib_full(ffi::gtk_icon_theme_get_search_path(
                self.to_glib_none().0,
            ))
        }
    }
