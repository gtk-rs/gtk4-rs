...
//! Traits intended for implementing the [`AccessibleText`](crate::AccessibleText) interface.
...
mod sealed {
    pub trait Sealed {}
    impl<T: super::AccessibleTextImplExt> Sealed for T {}
}

pub trait AccessibleTextImplExt: sealed::Sealed + ObjectSubclass {
    fn parent_attributes(&self, offset: u32) -> Vec<(AccessibleTextRange, GString, GString)> {
        unsafe {
            ...
            let mut n_ranges = MaybeUninit::uninit();
...
unsafe extern "C" fn accessible_text_get_extents<T: AccessibleTextImpl>(
    ...
    if let Some(rect) = rect {
        std::ptr::copy_nonoverlapping(rect.into_glib_ptr(), extents, 1);
